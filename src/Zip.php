...
class Zip
{
    ...
    public static function compress($source_path, $dest_path = '', $excludes = array()) {
        if (is_array($source_path)) {
            empty($dest_path) and $dest_path = './compressed_' . time() . '.' . self::ZIP_EXT;
            
            $zip = new ZipArchive();
            if ($zip->open($dest_path, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
                foreach ($source_path as $path) {
                    is_file($path) and $zip->addFile($path, basename($path));
                }
                $zip->close();
                return true;
            }else {
            ...
            empty($dest_path) and $dest_path = './' . $path_info['basename'] . '.' . self::ZIP_EXT;
    ...
    public static function extract($source_path, $dest_path = '') {
        ...
        if ($source_path = realpath($source_path) and $zip->open($source_path) === true) {
            empty($dest_path) and $dest_path = './' . pathinfo($source_path, PATHINFO_FILENAME);
    ...
    protected static function _processZip($path, &$zip, $path_pos, $excludes) {
        ...
        while (false !== $f = readdir($handle)) {
            if ($f == '.' or $f == '..' or ( is_array($excludes) and in_array($f, $excludes))) {
                continue;
            }
    ...
    protected static function _htmlList($path_tree) {
        ...
        if (!empty($path_tree)) {
            ...
            foreach ($path_tree as $name => $path) {
                ...
                if (is_array($path)) {
                    $html_list .= '<span class="directory-name">' . $name . '</span>' . self::_htmlList($path);
                } else {
                    $html_list .= '<span class="file-name">' . $name . '</span>';
                }
                $html_list .= '</li>';
            }
            $html_list .= '</ul>';
